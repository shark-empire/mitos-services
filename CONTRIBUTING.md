...
- **New parsing logic gets tests.** See the `#[cfg(test)]` modules in
  `config.rs`/`units.rs`/`cgroups.rs`/`users.rs`/`targets.rs`/
  `timers.rs`/`sandbox.rs`/`oom.rs`/`journal.rs` for the existing
  pattern. Logic that needs root or a real kernel (cgroups, the actual
  supervision loop, and `sandbox.rs`/`seccomp.rs`'s namespace/
  capability/syscall-filter setup) doesn't have automated tests yet;
  that's what VM testing is for. `seccomp.rs` specifically has never
  been checked against a real kernel at all - see its module doc before
  trusting it.
